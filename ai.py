...
Cosine similarity (косинусова прилика) е мярка за сходство между два вектора в многомерно пространство. Тя се базира на ъгъла между двата вектора, а не на тяхната големина. Изчислява се по формулата:

cos(𝜃) = 𝐴⋅𝐵 / ∥𝐴∥ ∥𝐵∥
където: A⋅B е скаларното произведение на векторите 
∥A∥ и ∥𝐵∥ са дължините (нормите) на векторите.
𝜃 е ъгълът между тях.

Защо е полезна?
Cosine similarity се използва често за сравняване на текстове, изображения и други видове данни, като се представят чрез вектори. Най-често срещани приложения:
✅ Търсене на документи (например Google търсене)
✅ Класифициране на текстове
✅ Препоръчващи системи (например Netflix, Spotify)

Пример с Python
Ето как можеш да изчислиш cosine similarity с NumPy и scikit-learn:

python
import numpy as np
from sklearn.metrics.pairwise import cosine_similarity

# Два примерни вектора
A = np.array([1, 2, 3])
B = np.array([4, 5, 6])

# Ръчно изчисление
cos_sim = np.dot(A, B) / (np.linalg.norm(A) * np.linalg.norm(B))
print(f"Cosine Similarity (ръчно): {cos_sim:.4f}")

# Използване на sklearn
cos_sim_sklearn = cosine_similarity([A], [B])[0][0]
print(f"Cosine Similarity (sklearn): {cos_sim_sklearn:.4f}")
Резултатът ще бъде число между -1 и 1:
1 означава, че векторите са идентични.
0 означава, че са напълно несвързани (ортогонални).
-1 означава, че са напълно противоположни.
Ако работиш с текст, обикновено думите се векторизират (например чрез TF-IDF) и тогава се изчислява cosine similarity.
