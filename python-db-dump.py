...
def dump_mysql(args):
    ...
    with open(args.out, "a") as f:
        ...
        for t in tables:
            ...
            col_cmd = ["mysql", "-u", args.user]
            if args.password:
                col_cmd.append(f"-p{args.password}")
            if args.host:
                col_cmd += ["-h", args.host]
            col_cmd += ["-N", "-e", col_query]

            col_output = run_cmd(col_cmd, capture_output=True)
            cols = [c.strip() for c in col_output.splitlines() if c.strip()]
...
def dump_pgsql(args):
    ...
    with open(args.out, "a") as f:
        ...
        for t in tables:
            ...
            col_output = run_cmd(col_cmd, capture_output=True, env=env)
            cols = [c.strip() for c in col_output.splitlines() if c.strip()]

            # 2) get values
            data_query = f'SELECT * FROM "{t}" ORDER BY 1 DESC LIMIT {args.rows};'
            ...
            res = run_cmd(data_cmd, capture_output=True, env=env)
            lines = res.splitlines()
...
def main():
    parser = argparse.ArgumentParser(
        description="Dump structure + last N rows of all tables."
    )
